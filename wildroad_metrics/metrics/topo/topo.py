...
import code
import random
# import showTOPO  # Disabled SVG rendering
...
try:
    ...
    Set = set  # Python 3 compatibility
from subprocess import Popen


def create_rtree_index():
    """Create an in-memory rtree index to avoid file locking issues."""
    p = index.Property()
...
def TOPOGenerateStartingPoints(OSMMap, check = True, density = 0.00070, region = None, image = None, direction = False, metaData = None, mergin=0.10):
    result = []
    ...
    # showTOPO.RenderRegion(  # Disabled SVG rendering


    pass  # showTOPO.RenderRegion(result, svgEdges, region, "gt.svg")
...
def TOPOGeneratePairs(GPSMap, OSMMap, OSMList, threshold = 0.00010, region = None, single = False, edgeids = None):
    ...
    if region is not None:
        # showTOPO.RenderRegion2(  # Disabled SVG rendering
        pass  # showTOPO.RenderRegion2(OSMList, matchedLoc, svgEdges, region, "coverage.svg")
...
def TOPO121(topo_result, roadgraph):
    # create index
    rtree_index = create_rtree_index()
...
def TOPOWithPairs(GPSMap, OSMMap, GPSList, OSMList, step = 0.00005, r = 0.00300, threshold = 0.00015, region = None, outputfile = "tmp.txt", one2oneMatching = True, metaData = None):
    ...
    for k,itemGPS in GPSList.items():
        ...
        idx_marbles = create_rtree_index()
        idx_holes = create_rtree_index()
        idx_holes_bidirection = create_rtree_index()
        ...
        for marble in marbles:
            ...
            for hole_id in possible_holes:
                ...
                if ddd < threshold and angle_d < 0.22:   # tighten angle (~12.6°)
        ...
        for hole in holes:
            ...
            for marble_id in possible_marbles:
                ...
                if ddd < threshold and angle_d < 0.22:
...
def TOPOWithPairsNew(GPSMap, OSMMap, GPSList, OSMList, step = 0.00005, r = 0.00300, threshold = 0.00015, region = None, outputfile = "tmp.txt", one2oneMatching = True, base_n = None, svgname = "", soft = True, CheckGPS = None):
    ...
    for k,itemGPS in GPSList.items():
        ...
        # showTOPO.RenderSVG(  # Disabled SVG rendering

        pass  # showTOPO.RenderSVG(marbles, holes_bidirection, matched_marbles,matched_holes,  lat, lon, 0.00500, "svg/"+svgname +str(i)+"_"+str(lat)+"_"+str(lon)+".svg")
...
def TOPOWithList(GPSMap, OSMMap, pairs, step = 0.00005, r = 0.00300, threshold = 0.00015, region = None, outputfile = "tmp.txt"):
    ...
    for min_node in pairs.keys():
        ...
        pass  # showTOPO.RenderSVG(marbles, holes, lat, lon, 0.00500, "svg/"+str(i)+"_"+str(lat)+"_"+str(lon)+".svg")
