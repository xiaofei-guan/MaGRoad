...
var interval_2 float64 = 20.0 // 40 meters for control points  (20*2=40)

var min_distance_filter float64 = 120.0 // don't consider shortest path shorter than 100.0
var prop_step int = 6 // one-to-one matching, if two control points mapped to A,B in the inferred graph, distance between A and B on the graph should greater than '4' hops, each hop is 2 meters.

var region_size_lat float64 = 1229.0 // unit is meter (latitude direction, i.e., height)
var region_size_lon float64 = 2458.0 // unit is meter (longitude direction, i.e., width)
var margin_size float64 = 80.0 // unit is meter 

var latTopLeft float64 = 41.0
var lonTopLeft float64 = -71.0

var effectiveMarginLat float64 = margin_size
var effectiveMarginLon float64 = margin_size
...
func GPSInBound(p1 [2]float64) bool {
	...
	latBottom := latTop - region_size_lat/111111.0
	lonRight := lonLeft + region_size_lon/111111.0/math.Cos(latTop/180.0*math.Pi)

	if p1[0] > latBottom+effectiveMarginLat/111111.0 &&
		p1[0] < latTop-effectiveMarginLat/111111.0 &&
		p1[1] > lonLeft+effectiveMarginLon/111111.0/math.Cos(latTop/180.0*math.Pi) &&
		p1[1] < lonRight-effectiveMarginLon/111111.0/math.Cos(latTop/180.0*math.Pi) {
		return true
	}
...
func updateRegionBounds(graphs ...*graph) {
	if len(graphs) == 0 {
		return
	}
	...
	minSampleLat := math.MaxFloat64
	maxSampleLat := -math.MaxFloat64
	minSampleLon := math.MaxFloat64
	maxSampleLon := -math.MaxFloat64
	...
	if minLat == math.MaxFloat64 || minLon == math.MaxFloat64 {
		return
	}
	...
	region_size_lat = (maxLat - minLat) * 111111.0
	if region_size_lat < 0 {
		region_size_lat = -region_size_lat
	}
	region_size_lon = (maxLon - minLon) * 111111.0 * math.Cos(latTopLeft/180.0*math.Pi)
	if region_size_lon < 0 {
		region_size_lon = -region_size_lon
	}

	if region_size_lat < 1.0 {
		region_size_lat = 1.0
	}
	if region_size_lon < 1.0 {
		region_size_lon = 1.0
	}

	latSpanSmall := (maxSampleLat - minSampleLat) * 111111.0
	lonSpanSmall := (maxSampleLon - minSampleLon) * 111111.0 * math.Cos(latTopLeft/180.0*math.Pi)

	smallestDim := region_size_lat
	if region_size_lon < smallestDim {
		smallestDim = region_size_lon
	}
	if latSpanSmall > 0 && lonSpanSmall > 0 {
		candidate := 0.25 * latSpanSmall
		if lonSpanSmall < latSpanSmall {
			candidate = 0.25 * lonSpanSmall
		}
		if candidate < smallestDim/4.0 {
			margin := candidate
			if margin < 30.0 {
				margin = 30.0
			}
			effectiveMarginLat = margin
			effectiveMarginLon = margin
		} else {
			effectiveMarginLat = smallestDim / 6.0
			effectiveMarginLon = smallestDim / 6.0
		}
	} else {
		effectiveMarginLat = margin_size
		effectiveMarginLon = margin_size
	}
}
...
func apls_one_way(graph_gt *graph, graph_prop *graph, ret chan float64) {
	...
	if len(control_point_gt_list) == 0 || len(control_point_prop_list) == 0 {
		fmt.Println("[Warning] No valid control point matches; returning 0.0")
		ret <- 0.0
		return
	}
...
func main() {
	...
	if len(os.Args) > 4 {
		...
		prop_step = 3
		margin_size = 30.0
		region_size_lat = 352.0
		region_size_lon = 352.0
	}

	updateRegionBounds(graph_gt, graph_prop)
