...
class GraphLabelGenerator():
    def __init__(self, config, full_graph, coord_transform):
        ...
        #   A dict. Keys are (row, col) coordinates of each node. Float inputs will be rounded to int.
        #   Values are lists, each item being a (row, col) of a neighbor node.
        # coord_transform: lambda, [N, 2] array -> [N, 2] array : yx -> xy
    ...
    def sample_patch(self, patch, rot_index = 0):
        ...
        patch_indices_all = set(self.graph_rtee.intersection(query_box)) # 找到所有属于query_box的点
        patch_indices = patch_indices_all - self.exclude_indices # 排除crossover附近的点
...
def custom_collate_fn(batch):
    ...
    # 构造符合要求的结构
    targets = [{'labels': torch.as_tensor(label), 'boxes': torch.as_tensor(keypoints)} for label, keypoints in zip(labels, keypoints)]

    for key in keys:
        if key != 'labels' and key != 'keypoints':
            collated[key] = torch.stack([item[key] for item in batch], dim=0)
...
class SatMapDataset(Dataset):
    def __init__(self, config, is_train, dev_run=False):
        ...
        if self.config.DATASET == 'cityscale':
            ...
            rgb_pattern = os.path.join(current_dir, './cityscale/20cities/region_{}_sat.png')
            keypoint_mask_pattern = os.path.join(current_dir, './cityscale/processed/keypoint_mask_{}.png')
            road_mask_pattern = os.path.join(current_dir, './cityscale/processed/road_mask_{}.png')
            gt_graph_pattern = os.path.join(current_dir, './cityscale/20cities/region_{}_refine_gt_graph.p')
    ...
    def __len__(self):
        if self.is_train:
            ...
            if self.config.DATASET == 'cityscale':
                ...
                return 128*24 # just for testing inference
            elif self.config.DATASET == 'spacenet':
                return 84667
        else:
    ...
    def __getitem__(self, idx):
        ...
        # 新增 keypoint 是归一化坐标
        # keypoints, label = self.extract_keypoints_from_mask(keypoint_mask_patch)
        path = "/home/guanwenfei/ResearchWork/sam_road-main/cityscale/keypoints/"
        keypoints = self.extract_keypoints_from_patch_image(patch,
                                                            path + f'region_{img_idx}.json',
                                                                   rot_index*90)
        if(len(keypoints) == 0):
            label = []
        else:
            label = [0]*len(keypoints)
        
        # rgb: [H, W, 3] 0-255
        # masks: [H, W] 0-1
        return {
            # 'rgb': torch.tensor(rgb_patch, dtype=torch.float32), # 思路1
            'rgb': torch.tensor(rgb_patch, dtype=torch.float32).permute(2, 0, 1), # 思路二
    ...
    def extract_keypoints_from_patch_image(self, patch:[(int, int), (int, int)],
                                           path:str, rotation_angle: int):

        # 读取JSON文件中的关键点
        with open(path, 'r') as f:
            data = json.load(f)
        keypoints = data['ke_coors']  # 假设关键点保存在keypoints字段中，格式为[(x, y), (x, y), ...]
        upper_left, lower_right = patch[0], patch[1]
        # 提取 upper_left 和 lower_right 的坐标
        x_min, y_min = upper_left
        x_max, y_max = lower_right

        # 筛选位于 upper_left 和 lower_right 矩形区域中的关键点
        selected_keypoints = [
            (x, y) for (x, y) in keypoints
            if x_min <= x <= x_max and y_min <= y <= y_max
        ]

        # 以 upper_left 为原点，调整关键点的坐标
        adjusted_keypoints = [
            (x - x_min, y - y_min) for (x, y) in selected_keypoints
        ]

        len_kp = len(adjusted_keypoints)

        rotation_angle = rotation_angle % 360
        if rotation_angle not in {0, 90, 180, 270}:
            raise ValueError("rotation_angle只能是0, 90, 180, 或 270")

        height, width = patch[1][1] - patch[0][1], patch[1][0] - patch[0][0]
        rotated_patch_keypoints = np.empty((0, 2))
        # 旋转坐标
        for x, y in adjusted_keypoints:
            if rotation_angle == 90:
                ...
                raise ValueError("rotation_angle只能是90, 180, 或 270")

            rotated_patch_keypoints = np.vstack([rotated_patch_keypoints, np.array([new_x, new_y])])

        if rotation_angle == 90:
            height, width = width, height

        rotated_patch_keypoints = np.array(rotated_patch_keypoints, dtype=np.float32)
        # if self.is_train is False:
            # 考虑shape为(0，2)时的情况
        #     print(rotated_patch_keypoints.shape)
        rotated_patch_keypoints[:, 0] /= width  # 归一化 x 坐标
        rotated_patch_keypoints[:, 1] /= height  # 归一化 y 坐标

        len_rot_kp = len(rotated_patch_keypoints)

        assert len_rot_kp == len_kp, "the lens of two lists are not the same!"

        return rotated_patch_keypoints


# 这个函数不合理，因为kp_mask为1的点和kp的数量不是一对一的，kp_mask是以kp为中心，radius=3做出来的
    def extract_keypoints_from_mask(self, keypoint_mask):
        height, width = keypoint_mask.shape

        # 使用 np.where 获取所有大于 0 的点的坐标
        y_coords, x_coords = np.where(keypoint_mask > 0)

        # 将坐标堆叠成 n 行 2 列的形式，每行是一个 [x, y] 坐标
        keypoints = np.stack((x_coords, y_coords), axis=1)
        keypoints_normalized = keypoints.astype(np.float32)
        keypoints_normalized[:, 0] /= width  # 归一化 x 坐标
        keypoints_normalized[:, 1] /= height  # 归一化 y 坐标

        label = np.ones(len(y_coords))
        print(label.shape, '!')
