...
def count_contiguous_ids(input_dir: str) -> int:
    """Count how many gt_graph_{i}.pickle files exist from 0..N-1 contiguously.

    Stops at the first missing id.
    """
    i = 0
    while True:
        p = os.path.join(input_dir, f'gt_graph_{i}.pickle')
...
def main():
    ...
    if total_tiles < 0:
        total_tiles = count_contiguous_ids(args.input_dir)
