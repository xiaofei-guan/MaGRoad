...
for tile_idx in [8, 9, 19, 28, 29, 39, 48, 49, 59, 68, 69, 79, 88, 89, 99, 108, 109, 119, 128, 129, 139, 148, 149, 159, 168, 169, 179]:
    ...
    # graph_gt = '../cityscale/20cities/region_%s_graph_gt.pickle'%tile_idx
    graph_prop = f'test_data/magtoponet_inference_20250902_180058_V9_RM_0.404/graph/{tile_idx}.p' # new
    graph_gt = f'test_data/20cities_gt_graph/region_{tile_idx}_graph_gt.pickle' # new
    # TODO(congrui): why modify args? 
    # args.output = '../%s/results/topo/%s.txt'%(args.savedir,tile_idx)
    args.output = f'exp/V9_RM_0.404_origi_topo/region_{tile_idx}_topo.txt' # new
    output_dir = os.path.dirname(args.output)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    print(f'Processing tile {tile_idx}...') # new
