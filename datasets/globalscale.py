...
class GlobalScaleSatMapDataset(Dataset):
    def __init__(self, config, is_train, dev_run=False):
        self.config = config
        self.is_train = is_train

        # Dataset-specific configuration
        dataset_name = getattr(config, 'DATASET', 'globalscale')
        ...
        if dev_run:
            tile_indices = tile_indices[:min(len(tile_indices), 200)]

        # Paths and GLG objects
        self.rgb_paths, self.keypoint_mask_paths, self.road_mask_paths = [], [], []
        self.graph_label_generators = []
        
        processed_data = 'train' if self.is_train else 'test'
        print(f'-------------------loading {processed_data} data------------------')

        for tile_id in tile_indices:
            print(f'loading tile {tile_id}')

            rgb_path = rgb_pattern.format(tile_id)
            keypoint_mask_path = keypoint_mask_pattern.format(tile_id)
            road_mask_path = road_mask_pattern.format(tile_id)
            tile_dir = os.path.join(precomputed_dir, f'tile_{tile_id}')
