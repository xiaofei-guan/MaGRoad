...
class GraphLabelGenerator():
    def __init__(self, config, full_graph, coord_transform):
        ...
        self.full_graph_subdivide = graph_utils.subdivide_graph(self.full_graph_origin, self.subdivide_resolution) # this function subdivides each edge into multiple segments, generates new points through interpolation, and updates the graph structure
        ...
        self.sample_weights[list(interesting_indices)] = config.INTR_SAMPLE_WEIGHT  # Both itsc and neighbor are interesting points
    ...
    def sample_patch(self, patch, rot_index=0):
        ...
        nms_scores = np.maximum(nms_scores, nms_score_override) # The scores of sampled points are between 0.9 and 1.0, and the score of intersection is 2.0
...
class WildRoadSatMapDataset(Dataset):
    def __init__(self, config, is_train, dev_run=False):
        self.config = config
        self.is_train = is_train

        # wildroad specific configuration
        self.IMAGE_SIZE = 1024  # wildroad uses 1024x1024 patches
        self.SAMPLE_MARGIN = config.SAMPLE_MARGIN
        
        base_dir = './wildroad'
        glg_base_dir = os.path.join(base_dir, 'wild_road_GLG')
    ...
    def __len__(self):
        if self.is_train:
            # For wildroad: return number of tiles (each tile is 1024x1024)
